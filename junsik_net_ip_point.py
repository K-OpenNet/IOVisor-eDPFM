...
# newly added :
# ether_dst_addr, ether_src_addr
# ip_ttl, ip_header_len, ip_flags, ip_protocol
# tcp_segment_len, tcp_header_len, tcp_window_size, tcp_checksum, tcp_urgent_pointer
...
import json
import yaml
# added by young eun - BEGIN
import time
# added by young eun - END
...
class NetworkIpPacketPoint:
    ...
    # set option stream to view log - START
    
    def set_option_stream(self):       
        self._option["output_type"] = "stream"
        
    # set option stream to view log - END
    ...
    def _init_bpf(self):
        ...
        self.tester = self._bpf_bytecode.trace_print() # THIS WAY THE MESSAGE IS PRINTED
    ...
    def _store_packet_ethernet_info(self, packet_bytearray, packet_info):
        ethernet_length = total_length
        packet_info["ethernet_length"] = total_length
...
if __name__ == "__main__":
    ...
    point.set_option_stream()
    point.collect()
    #point.bpf_trace_printk()
